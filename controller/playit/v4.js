...
class PlayItClient extends EventEmitter {
  ...
  login() {
    ...
    prs.stdout.on("data", (data) => {
      ...
      setTimeout(() => {
        ...
        exec(`${plyPath} claim exchange "${claimCode}"`, (err, stdout, stderr) => {
          if (err) {
  ...
  start() {
    const p = spawn(plyPath, ["start"], { stdio: "inherit" });
    this.emit('starting');
...
const fmm = fileURLToPath(import.meta.url).toLowerCase().replace(".js", "");
...
if (process.argv[1] === fmm) {
  ...
  switch (arg) {
    ...
    case "--list": client.listTunnels(); break;
    case "--start": client.start(); break;
    case "--reset": client.reset(); break;
    case "--secret-path": client.showSecretPath(); break;
    case "--version": client.showVersion(); break;
    case "--help":
    default: client.help(); break;
  }
}
